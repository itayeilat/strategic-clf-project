...
class WeightedLinearCostFunction(SeparableCost):
    def __init__(self, weighted_vector: np.array):
...
class MixWeightedLinearSumSquareCostFunction(CostFunction):
    def __init__(self, weighted_vector: np.array, epsilon=0.3):
        self.a = weighted_vector
    ...
    def maximize_features_against_binary_model(self, x: np.array, trained_model,tolerance=1e-9):
        ...
        # cost_factor = 10
        func_to_solve = cp.Minimize(cost_factor*(cp.maximum((1 - self.epsilon) * self.a.T @ (x_tag - x), 0) + self.epsilon *
                                               cp.sum((x_tag - x) ** 2)))
